-- Bound extremely broad full-text queries so common words cannot exceed the
-- public API timeout. Exact name matches are always included before the
-- bounded GIN candidate pool; normal queries retain their exact total.
...
as $$
  ...
  ), imported_match_ids as materialized (
    select r.import_id, r.source_product_id
    from public.food_dataset_compact_records r
    where r.import_id = (select id from active)
      and public.food_immutable_unaccent(lower(r.name)) = (select term from parameters)
    union
    select bounded.import_id, bounded.source_product_id
    from (
      select r.import_id, r.source_product_id
      from public.food_dataset_compact_records r
      where r.import_id = (select id from active)
        and r.search_document @@ (select query from parameters)
      limit 1000
    ) bounded
  ), candidate_ids as (
    select
